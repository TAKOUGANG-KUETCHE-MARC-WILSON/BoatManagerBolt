...
async function getEntretienCategoryId() {
  const { data, error } = await supabase
    ...
    .select('id')
    .ilike('description1', 'Représentation') // insensible à la casse, exact
    .limit(1)
    .maybeSingle();

  if (error || !data) {
    throw new Error('La catégorie "Représentation" est introuvable dans categorie_service.description1');
  }
