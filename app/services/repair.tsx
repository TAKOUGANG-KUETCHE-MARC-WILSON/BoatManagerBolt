...
async function getEntretienCategoryId() {
  const { data, error } = await supabase
    ...
    .select('id')
    .ilike('description1', 'Réparation / Panne') // insensible à la casse, exact
    .limit(1)
    .maybeSingle();

  if (error || !data) {
    throw new Error('La catégorie "Réparation / Panne" est introuvable dans categorie_service.description1');
  }
...
export default function RepairScreen() {
  ...
  const handleSubmit = async (formData: any) => {
    ...
    const issueType = formData.issueType ? formData.issueType.trim() : '';
    let description = issueType;

    if (detailedDescription.trim()) {
      description += (description ? ' : ' : '') + detailedDescription.trim();
    }
