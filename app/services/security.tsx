...
async function getEntretienCategoryId() {
  const { data, error } = await supabase
    ...
    .select('id')
    .ilike('description1', 'Sécurité') // insensible à la casse, exact
    .limit(1)
    .maybeSingle();

  if (error || !data) {
    throw new Error('La catégorie "Sécurité" est introuvable dans categorie_service.description1');
  }
...
// Extracted UrgencySelector component for consistency with other service forms
...
export default function SecurityScreen() {
  ...
  const handleSubmit = async (formData: any) => {
    ...
    const description = formData.securityDetails ? formData.securityDetails.trim() : '';

    if (!description) {
      Alert.alert('Erreur', 'Veuillez décrire les instructions de sécurité.');
      return;
    }
          ...
          fields={[
            {
              ...
              icon: Tool, // Using Tool icon for consistency
              multiline: true,
            },
          ]}
          submitLabel="Envoyer"
