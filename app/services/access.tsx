...
// Résout un id de catégorie depuis categorie_service.description1
async function getEntretienCategoryId() {
  const { data, error } = await supabase
    .from('categorie_service')
    .select('id')
    .ilike('description1', 'Gestion des accès') // pas de %, => égalité insensible à la casse
    .maybeSingle();

  if (error || !data) {
    throw new Error('La catégorie "Gestion des accès" est introuvable dans categorie_service.description1');
  }
...
export default function AccessScreen() {
  ...
  if (submitting) return; // évite les doubles envois
  ...
  if (!description.trim()) {
    Alert.alert('Erreur', 'Veuillez sélectionner au moins un besoin ou décrire votre demande.');
    return;
  }

  setSubmitting(true);
  try {
    // Résoudre la catégorie (tu gardes ta fonction)
    const id_service = await getEntretienCategoryId();

    // Choisir le boat manager (tu gardes ta fonction)
    ...
    Alert.alert('Erreur', e instanceof Error ? e.message : 'Une erreur inattendue est survenue.');
  } finally {
    setSubmitting(false);
  }
};
