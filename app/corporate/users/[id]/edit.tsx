...
import { ArrowLeft, User, Mail, Phone, Building, Shield, ImageIcon, X, Check } from 'lucide-react-native';
...
const getRoleName = (role: CorporateUser['role']): string => {
  return roleOptions.find(option => option.value === role)?.label || role;
};

const getRoleColor = (role: CorporateUser['role']): string => {
  switch (role) {
    case 'super-admin':
      return '#F59E0B';
    case 'admin':
...
// Mock data for corporate users (replace with actual API call in production)
const mockCorporateUsers: Record<string, CorporateUser> = {
  'cu1': {
    id: 'cu1',
    firstName: 'Admin',
    lastName: 'YBM',
...
export default function EditCorporateUserScreen() {
  ...
  const { user } = useAuth();
  const [formData, setFormData] = useState<Partial<CorporateUser>>({
    firstName: '',
    lastName: '',
    email: '',
    phone: '',
  ...
  const [errors, setErrors] = useState<Partial<CorporateUser>>({});
  const [showPhotoModal, setShowPhotoModal] = useState(false);
  const [showRoleModal, setShowRoleModal] = useState(false);
  const [mediaPermission, requestMediaPermission] = ImagePicker.useMediaLibraryPermissions();
  const [isLoading, setIsLoading] = useState(true); // Set to true initially for data loading

  useEffect(() => {
    if (id && typeof id === 'string' && mockCorporateUsers[id]) {
      const corporateUser = mockCorporateUsers[id];
      setFormData({
      ...
      Alert.alert(
        'Erreur',
        'Utilisateur Corporate non trouvé',
        [
          {
            text: 'OK',
            onPress: () => router.back()
  ...
  const validateForm = () => {
    const newErrors: Partial<CorporateUser> = {};
