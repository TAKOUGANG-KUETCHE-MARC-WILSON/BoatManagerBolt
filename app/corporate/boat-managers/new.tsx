...
import { ArrowLeft, User, Mail, Phone, MapPin, Check, X, Plus, Key, Send, Lock, Ship, Briefcase } from 'lucide-react-native';
...
export default function NewBoatManagerScreen() {
  const { user } = useAuth();
  ...
  const [selectedPorts, setSelectedPorts] = useState<Port[]>([]); // Changed from portAssignments
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        // 2. Create the new boat manager user
        const { data: newUser, error: userInsertError } = await supabase
          ...
          .insert({
            ...
            profile: 'boat_manager',
            status: 'active', // New users are active by default
            last_login: new Date().toISOString(), // Set initial last_login
            avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=987&auto=format&fit=crop', // Default BM avatar
          })
          .select('id')
          .single();
        ...
        const userPortInserts = selectedPorts.map((port) => ({ // Use selectedPorts directly
          user_id: newUser.id,
        ...
        Alert.alert(
          'Succès',
          'Le Boat Manager a été ajouté avec succès.',
          [
            {
              text: 'OK',
              onPress: () => router.back()
        ...
        Alert.alert('Erreur', error.message || 'Une erreur est survenue lors de l\'ajout du Boat Manager.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Ajouter un Boat Manager</Text>
      </View>
        ...
        <View style={styles.formContainer}>
          ...
          <View style={styles.formSection}>
            <View style={styles.sectionHeader}>
              ...
              <TouchableOpacity 
                ...
                onPress={handleAddPortSelection} // Changed to open multi-select modal
              >
            ...
            {selectedPorts.length > 0 ? ( // Display selected ports as tags
            ...
            <View style={styles.optionRow}>
              <View style={styles.optionInfo}>
                ...
                <Text style={styles.optionDescription}>
                  Un email contenant les identifiants de connexion sera envoyé au Boat Manager
                </Text>
              </View>
              <Switch
          ...
          >
            {isLoading ? (
              ...
              <>
                <Check size={20} color="white" />
                <Text style={styles.submitButtonText}>Ajouter le Boat Manager</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
